...
| `tools/checker` | The batch checker: slices 0 and 1 of `SEMANTIC_DESIGN.md`. A binary that loads a package and everything it `use`s, and rejects on parse diagnostics, over-deep nesting, `use`-level legality and resolution errors, ponyc's syntax-pass legality rules, and unresolved names, in ponyc's own wordings. |
...
The batch checker covers its first two slices. Slice 0: the driver, the
loader, a parser depth guard that turns over-deep nesting into a
diagnostic instead of a crash — every grammar recursion cycle carries a
guard, which `make test` re-derives from the source — and ponyc's syntax
pass ported in full, plus the package-docstring rule from its sugar
pass — this implementation has no pass boundary to stop at, decided by
Red. Slice 1: name resolution — a reference, member, or type that does
not resolve fails the case, in ponyc's wording with its did-you-mean
suggestion. Lookups fail open: a name not provably unresolvable is
accepted, so a resolver gap costs a missed rejection rather than a
false one. On the corpus's valid cases the checker rejects nothing
ponyc accepts, and it catches every rejection ponyc makes at its
parse or syntax pass; the remaining disagreements need later passes,
or the parts of ponyc's name passes the checker does not prove.
